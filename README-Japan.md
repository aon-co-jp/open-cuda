...
v0.3.5 では、v0.3.4 で実機 Vulkan Compute の `vector_add` が成功した状態を前提に、警告削除、PowerShell 実行ポリシー回避用 `.cmd`、通常テスト/実Vulkanテストのスクリプトを追加しました。
...
実Vulkanの `vector_add` は、PowerShell実行ポリシーを避けたい場合、次の `.cmd` が使えます。
...
> ⚠️ **v0.3.5 — CPU + Mock + 最小実Vulkan.** これは設計と動くプロトタイプを示す初期リリースです。
> 現時点で実際に動くのは **CPUバックエンド**（GPU不要）、GPUなしでSPIR-V/OmniIR経路を検証する **Mockバックエンド**、そして `vector_add` に限定した **実Vulkan Computeバックエンド** です。
> まだ高性能GPUランタイムではありません。誇大な約束はしません。代わりに、
> 「今どこまで動くか」と「どこを目指すか」を明確に分けて書きます。

[![License](https://img.shields.io/badge/license-Apache--2.0-blue.svg)](LICENSE)
[![Status](https://img.shields.io/badge/status-v0.3.5%20prototype-orange.svg)]()
...
詳細は `README-VULKAN.md` を参照してください。
...
# v0.3.5: 同じOmniIRをCPU NativeとVulkanMockで検証
cargo run --release -p vector_add_omniir
...
OpenCUDA v0.3.5 は「本物Vulkan実装」の直前に必要な、GPUなしで壊れにくい設計を固める版です。
...
- v0.3.5 の `opencuda-vulkan` はまだ実Vulkanではありません。
