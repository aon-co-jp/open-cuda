...
> **آخر تحديث (2026-08-20)**: تم تنفيذ تكميم INT6 على طراز FlexQ
> (`quantize_int6`/`dequantize_int6`/`QuantizedInt6Tensor` في
> `opencuda-blas`، يحزم 4 قيم 6-بت في 3 بايتات). تم رفض PuzzleMoE بعد
> التحقق من متطلباته المسبقة — يتطلب بنية MoE موجودة مسبقًا لا تملكها
> نماذج GPT-2/BERT الكثيفة في هذا المستودع. كما تم تصحيح ملاحظة سابقة
> غير دقيقة تفيد بأن `open-directx` و`open-cuda` بينهما "ارتباط قيد
> التصميم" — في الواقع هما مشروعان منفصلان بنفس الاسم فقط: حزمة
> `opencuda-directx` الداخلية والمستودع المستقل
> `aon-co-jp/open-directx`. كما تم بحث ورفض إضافة آلية تحديث تلقائي
> (نمط `self_update.rs` من `open-english`) — هذا المستودع لا يملك
> خدمة مقيمة (10 حزم مكتبية + 12 ملف تنفيذي مثال يُستخدم مرة واحدة)،
> وتم تأكيد أن DirectX/CUDA الحقيقيين من Microsoft/NVIDIA هما بنيتا
> مكتبات وقت تشغيل وليسا خدمتي خلفية مقيمتين، لذا التصميم الحالي ليس
> عيبًا. راجع سجلات HANDOFF بتاريخ 2026-08-19/2026-08-20 في
> [CLAUDE.md](CLAUDE.md).
