...
(31.4GB・4分割)のダウンロードを開始し、実機検証を試みた**——ダウンロード
...
完了・ファイルサイズも期待値と完全一致を確認)を、新設した
`examples/deepseek_v2_lite_real_weights_probe.rs`で実際にロード・生成
してみた結果を正直に記録する。

**ロードは成功した**: `DeepseekModel::load`が**19.9秒**で完了。
`ModelWeights`のヘッダのみ読み込み+`ExpertSlot::Lazy`(実際に選ばれる
まで個々のエキスパートを読まない)という設計が、実際の31.4GBの
チェックポイントに対して実機で機能することを確認できた——これ自体は
明確な成果。

**しかし生成開始直後、メモリ危険域に到達した**: PowerShellの
`Get-Process`でこのプロセスの実メモリを5秒間隔で監視したところ、
生成開始からわずか数十秒で`WorkingSet`が約4.1GB→19.1GBまで急増し、
同時に`Get-CimInstance Win32_OperatingSystem`で計測したシステム全体の
空きメモリ(`FreePhysicalMemory`)が**127.5MB**まで低下した。あらかじめ
仕込んでおいた安全装置(空きメモリが2000MB未満になったら
`Stop-Process -Force`で強制終了する監視ループ)が作動し、プロセスを
自動的にkillした——クラッシュには至らず、kill直後にシステムの空き
メモリは約25GBまで正常に回復したことを確認した(安全装置が正しく
機能した)。

**推定される原因(2点、いずれも記録に値する)**:
1. MoE層(26層)それぞれで`num_experts_per_tok=6`個のエキスパートが
   独立に選ばれるため、生成1トークン目だけで最大`6×26=156`個の
   エキスパート読み込みが発生し得る——これは設計時に想定済みだった
   (156個×約35MB≈5.4GB)。
2. それに加えて、30GBという巨大なファイル群への散発的な`seek`+`read`
   の繰り返しにより、**Windowsのファイルシステムキャッシュ自体が
   数十GB規模に膨張**し、`FreePhysicalMemory`(真に空いている物理
   ページ)を圧迫した可能性が高い——このキャッシュは本来OSが必要に
   応じて回収可能なリソースだが、安全装置は「回収されるかどうか」を
   待たずに安全側に倒して即座にkillする設計にしていたため、実際に
   OOMに至ったかどうかは不明(kill後の即座の回復〈25GBまで復帰〉は、
   このキャッシュ圧迫説を支持する状況証拠)。

**結論・正直な評価**: 遅延ロード設計は「ロード」フェーズについては
明確に有効(実機で19.9秒・実データ一括読み込み無しを確認)だが、
「生成」フェーズはMoEの疎性を加味してもなお、この開発機(実測32GB
RAM)には荷が重いことが実測で判明した。「実機検証した」という結果を
誇張せず、**生成の完走は達成できなかった**という事実をそのまま記録する。

**次回への引き継ぎ**: (1) より小さいVRAM/RAM要求のMoEチェックポイント
