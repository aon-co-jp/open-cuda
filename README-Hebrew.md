...
> **עדכון אחרון (2026-08-20)**: מומשה קוונטיזציית INT6 בסגנון FlexQ
> (`quantize_int6`/`dequantize_int6`/`QuantizedInt6Tensor` ב-
> `opencuda-blas`, אורזת 4 ערכי 6-סיביות ל-3 בייטים). PuzzleMoE נדחה
> לאחר בדיקת התנאים המוקדמים שלו — הוא דורש ארכיטקטורת MoE קיימת
> שאינה קיימת במודלים הצפופים GPT-2/BERT של מאגר זה. כמו כן תוקנה
> הערה קודמת לא מדויקת שטענה ל"קישור בשלב תכנון" בין `open-directx`
> ל-`open-cuda` — למעשה מדובר בשני פרויקטים בעלי שם זהה ולא קשורים:
> ה-crate הפנימי `opencuda-directx` והמאגר העצמאי
> `aon-co-jp/open-directx`. בנוסף, נבדקה ונדחתה האפשרות להוסיף מנגנון
> עדכון אוטומטי (בסגנון `self_update.rs` מ-`open-english`) — למאגר
> זה אין שירות רזידנטי (10 crates ספרייה + 12 בינארי דוגמה
> חד-פעמיים), ואומת כי ה-DirectX/CUDA האמיתיים של Microsoft/NVIDIA
> עצמם בנויים כספריות זמן ריצה ולא כשירותי רקע רזידנטיים — כך
> שהעיצוב הנוכחי אינו פגם. ראו את רשומות ה-HANDOFF מתאריך
> 2026-08-19/2026-08-20 ב-[CLAUDE.md](CLAUDE.md).
